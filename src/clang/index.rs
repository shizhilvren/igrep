...
pub fn main(file: &str, dir: &str, debug: bool) -> Result<()> {
    ...
    let compilation_database = crate::clang::compilation_database::CompilationDatabase::from_directory(dir).unwrap();
    ...
    println!("dir: {:?}", cmd.get_directory());
    println!("file: {:?}", cmd.get_filename());
    let mut args = cmd.get_arguments();

    args.pop();
    println!("args: {:?}", &args);
    let mapped = cmd.get_mapped_source();
