...
export class LSPClient {
    ...
    private v86: V86;
    private id: number = 0;
    private responce: ResponseLog = new ResponseLog(this);
    public update(byte: number) {
        const char = String.fromCharCode(byte);
        if (char === '\n') {
            while (true) {
                if (this.isLog()) {
                    console.debug('log from LSP server:', this.buf, byte);
                    break;
                } else {
                    this.buf = this.responce.update(this.buf);
                    if (this.buf === "") {
                        break;
                    }
                }
    ...
    public start() {
        ...
        this.v86.serial0_send("clangd --log=info\n");
    }
    next() {
